...
#include "crow.h"
std::string translateT2M(std::string texto);
std::string message;
...
void maquinaEstados(int dev, char frase[]);
...
int main()
{
    ...
    #pragma omp parallel sections num_threads(5) private(tid) shared(dev, frase, botoes, switches, valorBotao, valorSwitch)
    {
        // Servidor
        #pragma omp section
        {
            ...
            ([frase](const crow::request& req){

                std::string method_name = crow::method_name(req.method);
                std::ostringstream os;
                if(method_name == "GET"){
                    os << "<html><h1>Mensagem Morse:</h1><h2>" << translateT2M(message) << "</h2><h3>"<< translateT2M(frase) << "</h3></html>" << std::endl;
                    ...
                    message = x["message"].s();
                    os << translateT2M(message) << std::endl;
                    return crow::response{os.str()};
                } 
